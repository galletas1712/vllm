...
class CudaCommunicator(DeviceCommunicatorBase):
    ...
    def _snapshot_checkpoint_prepare_comms(self):
        ...
        if self.ca_comm is not None:
            logger.info(
                "CudaCommunicator checkpoint prepare: closing non-NCCL custom allreduce"
            )
            self.ca_comm.close()
            self.ca_comm = None
        if self.symm_mem_comm is not None:
            logger.info(
                "CudaCommunicator checkpoint prepare: closing non-NCCL symmetric memory"
            )
            self.symm_mem_comm.close()
